...
RUN mkdir /opt/kafka &&\
	wget https://archive.apache.org/dist/kafka/$KAFKA_VERSION/kafka_2.9.1-$KAFKA_VERSION.tgz -P /opt/kafka
...
RUN tar zxvf /opt/kafka/kafka*.tgz -C /opt/kafka &&\
	sed -i 's/num.partitions.*$/num.partitions=3/g' /opt/kafka/kafka_2.9.1-$KAFKA_VERSION/config/server.properties
